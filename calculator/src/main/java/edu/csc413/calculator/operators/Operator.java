...
public abstract class Operator {
    ...
    public static Operator getOperator(String token) {
        return operators.get(token); // retrieve the value mapped by a operator token
    }
    ...
    public static boolean check(String token) {
        return operators.containsKey(token); // check whether a token is being mapped into the operators hash map or not
    }
